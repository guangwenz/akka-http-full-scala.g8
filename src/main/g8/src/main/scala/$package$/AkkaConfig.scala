...
object AkkaConfig {
    def config(actorSystemName: String): Config = { 
        ...
        sys.env.get("HOST_NAME").foreach{ bindHost =>
        ...
        sys.env.get("AKKA_SERVICE_NAME").foreach{ serviceName =>
